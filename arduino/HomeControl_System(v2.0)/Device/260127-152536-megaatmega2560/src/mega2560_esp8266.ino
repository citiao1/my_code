...
void setup() {
  ...
  dht.begin();
  pinMode(11,OUTPUT);//屋内灯光
  pinMode(8,OUTPUT);//屋内蜂鸣器
  pinMode(3,INPUT_PULLUP);//开灯按钮
...
void knod_pro(){
  ...
  if(last_light+4==light1||last_light-4==light1||last_light==light1)return;
  last_light=light1;
  knod_time=millis();
  statusChanged=true;
