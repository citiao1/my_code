...
{
    // spi初始化
    QSPI_InitConfig(TR_CLK, TR_MISO, TR_MOSI, QSPI4_CS_P02_15, 10 * 1000 * 1000, 3); // cs脚不对应，这里随机选择一个，实际用的软件cs
    // spi的cs初始化
...
void IR_Write_byte_4000(unsigned char *dat) // 发送默认4000个字节
{
    ...
    for (int fre = 0; fre < 125; fre++)
    {
        ...
        QSPI_ReadWriteNByte(QSPI4, buff, buff_V, 32); // 一次发送16字节的数据，spi最大的缓冲区位32个字节。
    }
    TR_CS_H;
    TR_wait_endSign(200);
}
void IR_Wirte_byte(unsigned char *dat, uint16_t len) // 发送指定长度，但是需要小于4000
{
    ...
    if (rem != 0) // 发送不够32个字节的数据
    {
        ...
        QSPI_ReadWriteNByte(QSPI4, buff, buff_V, rem);
    }
...
void TR_Write_Image_Pixle(unsigned char height, unsigned char width, unsigned char *Pixle)
{
    ...
    for (i = 0; i < frequency; i++)
    {
        ...
        QSPI_ReadWriteNByte(QSPI4, buff_T, buff_R, 32);
    }
    if (remainder != 0)
    {
        memset(buff_T, 0x00, 32);
        memcpy(buff_T, &img[total_bytes - remainder], remainder);
        QSPI_ReadWriteNByte(QSPI4, buff_T, buff_R, remainder);
    }
