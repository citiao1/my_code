...
physical K2 is black. The calibration results still require final verification.
...
- `Code/vehicle_gray.c/.h`: eight-channel ADC sampling, calibration snapshots
  and white=0/black=1000 normalization.
...
- `Code/board_io.c/.h`: verified K1/K2/K3 mapping PB15/PB14/PB16 and PA28 buzzer patterns.
...
- V14 yaw control is a position-form PID matching `diansai_test`: target minus
  ...
  150 degrees/s. Heading control remains disabled.
...
- Grayscale telemetry reports raw 12-bit ADC readings from 0 to 4095. V17 maps
  every channel independently with `(raw-white)*1000/(black-white)` and clamps
  the result to 0..1000. All eight black/white spans must be at least 32 ADC
  counts before `normalization_valid` becomes 1. References remain in RAM and
  must be recaptured after reset; no Flash format has been introduced yet.
...
- V16 configures K1/K2/K3 directly as PB14/PB15/PB16 pull-up inputs because the
  ...
  V18 corrects the real-board order and maps short presses to K1/PB15 white,
  K2/PB14 black and K3/PB16 gyroscope recalibration. Every action stops
  the motors first. White/black capture gives one beep, valid normalization
  then gives a separate three-beep pattern, and successful gyro calibration
  gives two beeps. Any 800 ms long press still stops without running short action.
- The PA28 buzzer is driven by a four-entry, non-blocking pattern queue. Its
  timing advances in the 10 ms application task and therefore cannot pause PID.
  The board material does not state its active level; V16 assumes active high
  through one macro and this polarity still needs an on-board check.
...
- V20 accepts `LINEPID,kp_milli,ki_milli,kd_milli`, `LINEDIFF,ratio_milli`,
  `LINECFG` and `LINE,enable,speed_mm_s`. Defaults are `400000,0,120000,650`,
  matching the active AI8051U `UserCode/APP/chassis.c` direction PID and
  differential limit. `LINE,1,100` starts at 100 mm/s after grayscale
...
`YAWPID`, `ENCZERO`, `IMUZERO`, `GRAYWHITE`, `GRAYBLACK` and `GRAY`.
...
& 'D:\Keil5\UV4\UV4.exe' -b `
  ...
  -o 'D:\my_code\my_code\diansai\MSPM0G3507_Diansai_Test\Keil\build_v21.log'
```

The latest V21 build result is `0 Error(s), 0 Warning(s)` and its boot signature
is `BOOT,MSPM0G3507_DIANSAI_TEST,V21,WHEELTEC,9600`. This is build verification;
...
V12 through V21 set `SPEED_FEEDFORWARD_ENABLED` to zero. The calibration data is
...
wheel speed control. V21 still enables exactly `VEHICLE_LOOP_YAW_RATE |
VEHICLE_LOOP_SPEED`; the grayscale module supplies the yaw-rate target directly,
so the unused heading-angle bit remains clear.
...
internally from m/s and PWM timer counts to mm/s and percent. V12 through V21 feed
...
While `DRV` is active, firmware sends an extended `SPD` frame every 250 ms. The bridge
publishes it to VOFA+ under the `speed:` FireWater prefix with left/right target,
actual, error, feedforward, PID correction, final PWM and gains, and under a
separate `yaw:` prefix with yaw target, actual, error and correction details.
Normal `TEL` and `STA` slow to 2000 ms in this mode for the 9600-baud budget.

While `LINE` is active, the same `SPD` frame runs every 500 ms and `LIN` every
200 ms to stay within the 9600-baud budget. The bridge exposes `LIN` as `line:`:
CH5 raw error, CH6 filtered error, CH7 direction PID output, CH8/CH9 target and
actual yaw rate, CH10 wheel correction, CH11/CH12 wheel targets, CH13 base speed,
CH14 differential ratio and CH15 normalized grayscale sum.

V21 appends CH16 recovery mode, CH17 recovery milliseconds and CH18 active black
channels. Modes are WAIT=0, TRACKING=1, GAP_HOLD=2, BLIND_TURN=3 and LOST=4.
...
consecutive 20 ms samples; broad all-black corner patterns do not end blind turn.
...
error, feedforward, PID contribution and final differential correction. In
VOFA's `speed:` list, left target/actual are CH2/CH3 and right target/actual are
CH8/CH9. In `yaw:`, target/actual are CH3/CH4.
