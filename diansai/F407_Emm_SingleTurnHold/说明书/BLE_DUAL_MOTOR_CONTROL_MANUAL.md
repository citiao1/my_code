...
电机串口和状态机已经合并到 `EMM_Run()`，主循环不阻塞地反复调用：
...
蓝牙 USART2 使用单字节中断接收。UART4 使用 Receive-to-IDLE DMA 接收电机位置帧；HAL 回调只保存数据和重启接收，帧解析与电机状态机均由 `EMM_Run()` 在主循环中执行。业务代码设置角度时只需调用 `EMM_SetAngle(90.0f, 120.0f)`。
...
| `Src/bluetooth_app.c` | USART2 单字节中断接收、文本组帧、SET/STAT 解析，并调用统一的 `EMM_SetAngle()` |
| `Src/motor_app.c` | 提供 `EMM_Init()`、`EMM_Run()`、`EMM_SetAngle()`，内部管理双电机状态机 |
| `Src/motor_uart.c` | 由电机模块内部使用的 UART4 Receive-to-IDLE DMA 接收和 S_CPOS 位置帧解析 |
