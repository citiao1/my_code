...
static void motor_state_init(void)
{
...
void EMM_Init(void)
{
  /* UART4 的 DMA 接收也属于电机库，外部不再单独初始化。 */
  motor_uart_init_internal();
  motor_state_init();
}
...
uint8_t EMM_SetAngle(float motor_1_degree, float motor_2_degree)
{
  ...
  if (!((motor_1_degree >= -360.0f) && (motor_1_degree <= 360.0f) &&
        (motor_2_degree >= -360.0f) && (motor_2_degree <= 360.0f)))
  {
    return 0U;
  }

  /* 外部只传角度；绝对位置脉冲、方向和双机同步由状态机处理。 */
  pulses = motor_1_degree * (float)MOTOR_PULSES_PER_REV / 360.0f;
  pending_target_pulses[0] =
    (int32_t)(pulses + ((pulses >= 0.0f) ? 0.5f : -0.5f));

  pulses = motor_2_degree * (float)MOTOR_PULSES_PER_REV / 360.0f;
...
{
  float error_1 = motor_angle[0] - motor_target_angle[0];
  float error_2 = motor_angle[1] - motor_target_angle[1];
  ...
  return ((motor_state == MOTOR_HOLDING) &&
          (motor_position_valid[0] != 0U) &&
          (motor_position_valid[1] != 0U) &&
...
static void motor_state_run(void)
{
  uint32_t now = HAL_GetTick();
  uint8_t index;
  uint8_t on_target;
  ...
  /*
   * 新的绝对角度优先级最高。上电回零阶段先保存，保持、运动或回正阶段
   * 会尽快用最新目标接管，不需要调用者了解内部状态编号。
   */
  ...
  switch (motor_state)
  {
    ...
    case MOTOR_HOLDING:
      if ((motor_pending_steps != 0U) && (waiting_position == 0U))
      {
        --motor_pending_steps;
...
void EMM_Run(void)
{
  /* 调用者只维护一个入口；DMA 数据解析和电机状态机都在内部运行。 */
