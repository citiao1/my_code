 ...
 * @note
 * 使用流程:
 * 1. GPIO初始化完成后调用OLED_Init()初始化OLED. 本工程使用PD11-PD14软件SPI
...
static void OLED_WriteByte(uint8_t data)
{
  ...
  for (bit = 0U; bit < 8U; ++bit)
  {
    ...
    HAL_GPIO_WritePin(OLED_DATA_GPIO_Port, OLED_DATA_Pin,
...
static void OLED_SendData(const uint8_t *data, uint16_t len)
{
