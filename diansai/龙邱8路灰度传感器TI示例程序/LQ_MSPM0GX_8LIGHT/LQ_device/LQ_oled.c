...
void OLED_ShowString(unsigned char y,unsigned char x,unsigned char *chr,unsigned char size1)
{
	...
	{
		if(size1==8)OLED_ShowChar(x,y*8,*chr,size1,mode);
		if(size1==12)OLED_ShowChar(x,y*16,*chr,size1,mode);
		if(size1==8)x+=6;
		else x+=size1/2;
		chr++;
  }
...
//num:汉字对应的序号
//mode:0,反色显示;1,正常显示
void OLED_ShowChinese(unsigned char y,unsigned char x,unsigned char num,unsigned char mode)
{
	y*=16;
...
/********************************************************************************************************************
【函数名称】 LQ_Test_OLED
【功能概述】 OLED 测试函数，用于对 OLED 进行测试操作
【输入参数】 无
【返 回 值】 无
【使用示例】 LQ_Test_OLED();
【注意事项】 无
