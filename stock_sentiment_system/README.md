...
- 使用机器学习逻辑回归模型预测下一交易日上涨/下跌概率，并展示回测准确率和主要影响因素。
...
5. 查看机器学习预测页签，说明下一交易日上涨概率、回测准确率和主要影响因素。
6. 说明综合评分模型：
...
系统没有使用外部训练好的黑箱模型，而是每次根据当前股票的历史 K 线自动训练一个轻量级模型，流程如下：

1. 从东方财富或新浪财经获取历史日 K 数据。
2. 自动计算机器学习特征，包括：
   - 1 日、3 日、5 日、10 日收益率
