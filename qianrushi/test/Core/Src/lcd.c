...
vu16 dummy;
char buffer1[20];
//static u8 currentHighlightLine = 0xFF;
/*******************************************************************************
...
void LCD_HighlightLine(u8 Line, char *ptr, vu16 highlightTextColor, vu16 highlightBackColor)
{
    
    vu16 saveTextColor = TextColor;
    vu16 saveBackColor = BackColor;
    
    
    if (highlightTextColor == 0 && highlightBackColor == 0)
    {
        highlightTextColor = White;      
        highlightBackColor = Magenta;    
    }
    
    
    LCD_SetTextColor(highlightTextColor);
    LCD_SetBackColor(highlightBackColor);
    
    
    //LCD_ClearLine(Line);
    
		int paddling = (20 - strlen(ptr)) / 2;
    snprintf(buffer1, 20, "%*s%s%*s", paddling, "", ptr,paddling, "");
    LCD_DisplayStringLine(Line, (u8*)buffer1);
	
    
    
    LCD_SetTextColor(saveTextColor);
    LCD_SetBackColor(saveBackColor);
}

// ????:????????(????+????)
void LCD_HighlightLineDefault(u8 Line, char *ptr)
{
    LCD_HighlightLine(Line, ptr, White, Magenta);
}	
/*void LCD_HighlightLineAutoClear(u8 Line, u8 *ptr, vu16 textColor, vu16 bgColor)
{
    // ????????,?????????(????)
    if (currentHighlightLine != 0xFF)
    {
        vu16 saveText = TextColor;
        vu16 saveBack = BackColor;
        
        // ??????????
        LCD_SetTextColor(DefaultTextColor); // ???????
        LCD_SetBackColor(DefaultBackColor); // ???????
        LCD_ClearLine(currentHighlightLine);
        LCD_DisplayStringLine(currentHighlightLine, oldPtr); // ???????????
        
        LCD_SetTextColor(saveText);
        LCD_SetBackColor(saveBack);
    }
    
    // ?????
    LCD_HighlightLine(Line, ptr, textColor, bgColor);
