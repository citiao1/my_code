...
/* USER CODE BEGIN 2 */
/*void Led_Disp(uint8_t ucLed)//0x01->0x0100 == pin8
{
	//先让所有的LED灯熄灭
	HAL_GPIO_WritePin(GPIOC,GPIO_PIN_13|GPIO_PIN_14|GPIO_PIN_15|GPIO_PIN_8
                          |GPIO_PIN_9|GPIO_PIN_10|GPIO_PIN_11|GPIO_PIN_12,GPIO_PIN_SET);
	...
	//再点亮想点亮的灯
	HAL_GPIO_WritePin(GPIOC,ucLed<<8,GPIO_PIN_RESET);//0x0100
	HAL_GPIO_WritePin(GPIOD,GPIO_PIN_2,GPIO_PIN_SET);
	HAL_GPIO_WritePin(GPIOD,GPIO_PIN_2,GPIO_PIN_RESET);
}*/


//函数名：key_scanf；按键扫描函数
//入口：无
//出口：uint8_t key_return,B1-2-3-4 按下按键返回1-2-3-4。无按键返回0
//功能：读取按键值,采用状态机方式，0-未按下，1——按下。
