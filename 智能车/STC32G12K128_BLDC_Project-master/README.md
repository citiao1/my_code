...
![image-20250102150125310](./Resource/image-20250102150125310.png)

![image-20250102150058315](./Resource/image-20250102150058315.png)
...
![image-20250102145245769](./Resource/image-20250102145245769.png)

系统启动初期，主函数率先对 STC32G 芯片的各个外设模块，如定时器、比较器、ADC、PWM 等进行初始化配置，确保其正常运行。同时，对软件资源，如变量初始化、PWM 占空比默认值、电机死区设置等进行处理。

![image-20250102145434268](./Resource/image-20250102145434268.png)
...
![image-20250102145608178](./Resource/image-20250102145608178.png)

在电机成功启动后，系统适时启用比较器中断。比较器中断的主要职责是检测电机的过零信号，一旦进入该中断，即表明成功检测到过零信号。由于电机启动后运行状态相对稳定，干扰因素相对较少，因此在电机启动后采用比较器中断判断过零信号具有较高的准确性与可靠性。通过精准检测过零信号，系统能及时准确掌握电机转子位置信息，为电机换相操作提供关键依据。

![image-20250102145639613](./Resource/image-20250102145639613.png)

PWMB 中断专注于高效处理电调输入信号。在中断服务程序中，通过精确测量与计算输入信号的高电平持续时间及整个周期时间，精准获取油门大小信息，为电机运行提供精准的动力控制指令。

![image-20250102145706380](./Resource/image-20250102145706380.png)

定时器 0 中断在电机运行过程中作用显著，主要用于电机换相与消磁处理。电机运行时，依据预先设定的换相策略与时序，当定时器 0 中断触发时，执行换相操作。同时，定时器 0 中断还负责对电机进行消磁处理，通过延时方式实现消磁。

![image-20250102145806862](./Resource/image-20250102145806862.png)

定时器 4 中断主要用于对电机换相过程的超时判断。在电机正常运行中，若因某种原因致使换相操作未能在预定时间内完成，定时器 4 中断将被触发。一旦进入该中断，即表示换相超时情况发生，此时停止电机运转并发出故障报警信号。

####  三、硬件调整指南

![img](./Resource/clip_image002.jpg)



 ![e8ea1624100522ad72529c74d20f11d](./Resource/e8ea1624100522ad72529c74d20f11d.png)

如前文所述，本次优化在硬件方面主要对旧版电调中的 C27、C28、C29 三颗滤波电容予以移除，这是新旧电调的唯一硬件差异。对于持有旧版电调的用户，若期望升级至优化版本，只需自行拆除这三颗电容，并更新相应程序代码，即可实现电调性能优化，使其与新版电调具备相同功能与性能表现。在拆除电容过程中，务必遵循操作规范，防止对电调其他部件造成损坏，确保硬件修改顺利推进。


#### 四、逐飞制作好的参考学习驱动板简介及无刷电机推荐

#### 4.1.基于STC无感无刷电机驱动学习板端口简介
