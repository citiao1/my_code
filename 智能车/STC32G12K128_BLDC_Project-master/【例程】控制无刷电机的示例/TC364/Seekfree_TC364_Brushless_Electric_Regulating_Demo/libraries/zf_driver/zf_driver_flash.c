...
* 2023-07-14       pudding            修复之前不完整的断言判断
...
//  参数说明      page_num      需要写入的页编号   参数范围0-31
...
// 函数简介     读取一页
// 参数说明     sector_num      仅可填写0  此处扇区编号并无实际作用，只是留出接口
// 参数说明     page_num        当前扇区页的编号   参数范围 <0 - 31>
// 参数说明     buf             需要读取的数据地址   传入的数组类型必须为uint32
// 参数说明     len             需要写入的数据长度   参数范围 1-512
// 返回参数     void
// 使用示例     flash_read_page(0, 11, data_buffer, 512);
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
...
//  参数说明      len             需要写入的数据长度   参数范围 1-512
//  返回参数      void
//  使用示例      flash_write_page(0, 0, buf, 512);
//  备注信息
//-------------------------------------------------------------------------------------------------------------------
...
// 使用示例     flash_write_page_from_buffer(0, 31);
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
