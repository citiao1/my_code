...
// INT_SERVICE：     宏定义决定中断由谁处理，也称为服务提供者（在TC264中，中断被叫做服务），可设置范围IfxSrc_Tos_cpu0 IfxSrc_Tos_dma  不可设置为其他值
// 如果INT_SERVICE设置为IfxSrc_Tos_dma的话，ISR_PRIORITY的可设置范围则是0-16。


//------------PIT中断参数相关定义------------
#define CCU6_0_CH0_INT_SERVICE	IfxSrc_Tos_cpu0	    // 定义CCU6_0 PIT通道0中断服务类型，即中断是由谁响应处理 IfxSrc_Tos_cpu0 IfxSrc_Tos_dma 不可设置为其他值
...
#define EXTI_CH0_CH4_INT_SERVICE IfxSrc_Tos_cpu0	// 定义ERU通道0和通道4中断服务类型，即中断是由谁响应处理 fxSrc_Tos_cpu0 IfxSrc_Tos_dma  不可设置为其他值
...
#define EXTI_CH2_CH6_INT_PRIO  	5	                // 定义ERU通道2和通道6中断优先级 优先级范围0-16 越大优先级越高
...
//------------DMA中断参数相关定义------------
#define	DMA_INT_SERVICE     IfxSrc_Tos_cpu0	        // ERU触发DMA中断服务类型，即中断是由谁响应处理 fxSrc_Tos_cpu0 IfxSrc_Tos_dma  不可设置为其他值
#define DMA_INT_PRIO  	    60	                    // ERU触发DMA中断优先级 优先级范围1-255 越大优先级越高 与平时使用的单片机不一样


//------------串口中断参数相关定义------------
#define	UART0_INT_SERVICE       IfxSrc_Tos_cpu0	    // 定义串口0中断服务类型，即中断是由谁响应处理 fxSrc_Tos_cpu0 IfxSrc_Tos_dma  不可设置为其他值
