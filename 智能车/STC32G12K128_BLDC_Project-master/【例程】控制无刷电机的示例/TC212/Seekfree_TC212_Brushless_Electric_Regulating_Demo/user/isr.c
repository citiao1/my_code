...
// **************************** 外部中断函数 ****************************
IFX_INTERRUPT(eru_ch0_ch4_isr, 0, EXTI_CH0_CH4_INT_PRIO)
{
    interrupt_global_enable(0);                     // 开启中断嵌套
...
IFX_INTERRUPT(eru_ch1_ch5_isr, 0, EXTI_CH1_CH5_INT_PRIO)
{
    interrupt_global_enable(0);                     // 开启中断嵌套
...
//由于摄像头pclk引脚默认占用了 6通道，用于触发DMA，因此这里不再定义中断函数
//IFX_INTERRUPT(eru_ch2_ch6_isr, 0, EXTI_CH2_CH6_INT_PRIO)
//{
...
IFX_INTERRUPT(eru_ch3_ch7_isr, 0, EXTI_CH3_CH7_INT_PRIO)
{
    interrupt_global_enable(0);                     // 开启中断嵌套
	if(exti_flag_get(ERU_CH3_REQ15_P14_1))          // 通道3中断
	{
	    exti_flag_clear(ERU_CH3_REQ15_P14_1);
