...
void ips114_init_simspi(void)
{
    ...
    if      (IPS114_DISPLAY_DIR==0)    ips114_writeData_simspi(0x00);
    else if (IPS114_DISPLAY_DIR==1)    ips114_writeData_simspi(0xC0);
    else if (IPS114_DISPLAY_DIR==2)    ips114_writeData_simspi(0x70);
    else                            	ips114_writeData_simspi(0xA0);
...
void ips114_showint8_simspi(uint16 x,uint16 y,int8 dat)
{
	...
	while(i<3)
	{
		ips114_showchar_simspi(x+(8*(i+1)),y*16,'0' + a[i]);
		i++;
	}
}
...
void ips114_showuint8_simspi(uint16 x,uint16 y,uint8 dat)
{
	...
	while(i<3)
	{
		ips114_showchar_simspi(x+(8*i),y*16,'0' + a[i]);
		i++;
	}
