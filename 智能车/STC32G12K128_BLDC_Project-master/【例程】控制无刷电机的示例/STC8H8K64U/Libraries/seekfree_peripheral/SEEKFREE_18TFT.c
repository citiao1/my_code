...
{	
	...
	lcd_writeData_simspi(x_start+2);
	lcd_writeData_simspi(0x00);
	lcd_writeData_simspi(x_end+2);

	lcd_writeIndex_simspi(0x2b);
	lcd_writeData_simspi(0x00);
	lcd_writeData_simspi(y_start+1);
	lcd_writeData_simspi(0x00);
	lcd_writeData_simspi(y_end+1);	
...
void lcd_showint8_simspi(uint16 x,uint16 y,int8 dat)
{
	...
	while(i<3)
	{
		lcd_showchar_simspi(x+(8*(i+1)),y*16,'0' + a[i]);
		i++;
	}
}
...
void lcd_showuint8_simspi(uint16 x,uint16 y,uint8 dat)
{
	...
	while(i<3)
	{
		lcd_showchar_simspi(x+(8*i),y*16,'0' + a[i]);
		i++;
	}
